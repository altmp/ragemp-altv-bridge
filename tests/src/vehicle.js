...
describe('vehicle', () => {
    describe('server', () => {
        ...
        describe(null, () => {
            ...
            afterEach(async ({ client, server }) => {
                await server(async ({mp}) => {
                    for (let vehicle of mp.vehicles.toArray()) {
                        vehicle.destroy();
                    }
                    mp.vehicles.toArray().should.be.empty;
                    mp.vehicles.length.should.equal(0);
            ...
            it('should spawn invalid model properly', async ({server, client, player}) => {
                ...
                await client(async ({mp}) => {
                    await tryFor(() => mp.vehicles.length.should.equal(1));
                    const veh = mp.vehicles.toArray()[0];
                    await tryFor(() => veh.handle.should.not.equal(0));
                });
            });
        });
        ...
        describe('player', () => {
            ...
            let startEnterVehicleEmittedTimes = 0;
            let startEnterVehicleSeat = -1;
            let enterVehicleEmittedTimes = 0;
            let enterVehicleSeat = -1;
            let exitVehicleEmittedTimes = 0;
            let exitVehicleSeat = -1;

            beforeAll(async ({server, client, player, params}) => {
                await server(async ({mp}) => {
                    player.spawn(new mp.Vector3(0, 0, 73));
                    mp.vehicles.toArray().should.be.empty;
                    veh = mp.vehicles.new(params.model ?? 't20', new mp.Vector3(2, 2, 73), {dimension: player.dimension});
                    ...
                    mp.events.add('playerStartEnterVehicle', handler = (currentPlayer, vehicle, seat) => {
                        if (currentPlayer !== player || vehicle !== veh) return console.log('duh4');
                        startEnterVehicleEmittedTimes++;
                        startEnterVehicleSeat = seat;
                    });
                    handlers.push(['playerStartEnterVehicle', handler]);

                    mp.events.add('playerEnterVehicle', handler = (currentPlayer, vehicle, seat) => {
                        if (currentPlayer !== player || vehicle !== veh) return console.log('duh5');
                        enterVehicleEmittedTimes++;
                        enterVehicleSeat = seat;
                    });
                    handlers.push(['playerEnterVehicle', handler]);

                    mp.events.add('playerExitVehicle', handler = (currentPlayer, vehicle, seat) => {
                        if (currentPlayer !== player || vehicle !== veh) return console.log('duh6');
                        exitVehicleEmittedTimes++;
                        exitVehicleSeat = seat;
                    });
                    handlers.push(['playerExitVehicle', handler]);
                });
                ...
                await client(async ({mp}) => {
                    ...
                    mp.events.add('playerStartEnterVehicle', handler = (vehicle, seat) => {
                        if (vehicle !== veh) return;
                        startEnterVehicleEmittedTimes++;
                        startEnterVehicleSeat = seat;
                    });
                    handlers.push(['playerStartEnterVehicle', handler]);

                    mp.events.add('playerEnterVehicle', handler = (vehicle, seat) => {
                        if (vehicle !== veh) return;
                        enterVehicleEmittedTimes++;
                        enterVehicleSeat = seat;
                    });
                    ...
                    mp.events.add('playerLeaveVehicle', handler = (vehicle, seat) => {
                        if (vehicle !== veh) return;
                        exitVehicleEmittedTimes++;
                        exitVehicleSeat = seat;
                    });
                    handlers.push(['playerLeaveVehicle', handler]);
                });
            });
            ...
            afterAll(async ({server, client, player}) => {
                await server(async ({mp}) => {
                    ...
                    for (let handler of handlers) {
                        mp.events.remove(handler[0], handler[1]);
                    }
                    handlers = [];
                });
            ...
            it('should enter vehicle', async ({server, client, player}) => {
                await client(async ({mp}) => {
                    mp.game.task.enterVehicle(player.handle, veh.handle, 5000, -1, 2, 1, 0);
                    await tryFor(() => player.vehicle.should.exist, 8000);
                });
            ...
            it('should emit vehicle start enter event', async ({server, client, player}) => {
                await client(async ({mp}) => {
                    await tryFor(() => startEnterVehicleEmittedTimes.should.equal(1));
                });
            ...
            it('should emit correct seat id in start enter event', async ({server, client, player}) => {
                await server(async ({mp}) => {
                    startEnterVehicleSeat.should.equal(0);
                });
            });

            it('should emit vehicle enter event', async ({server, client, player}) => {
                await client(async ({mp}) => {
                    await tryFor(() => enterVehicleEmittedTimes.should.equal(1));
                });
            ...
            it('should emit correct seat id in enter event', async ({server, client, player}) => {
                await client(async ({mp}) => {
                    enterVehicleSeat.should.equal(0);
                });
            ...
            it('should shuffle seat', async ({server, client, player}) => {
                await client(async ({mp}) => {
                    mp.game.task.shuffleToNextVehicleSeat(player.handle, veh.handle, 0);
                    await waitFor(() => !player.getIsTaskActive(165), 4000);
                });
            });

            it('should emit vehicle enter event on shuffle', async ({server, client, player}) => {
                await client(async ({mp}) => {
                    await tryFor(() => enterVehicleEmittedTimes.should.equal(2), 5000);
                });

                await server(async ({mp}) => {
                    await tryFor(() => enterVehicleEmittedTimes.should.equal(2));
                });
            });

            it('should emit correct seat id on shuffle in enter event', async ({server, client, player}) => {
                await client(async ({mp}) => {
                    enterVehicleSeat.should.equal(1);
                });
            ...
            it('should return correct seat after shuffle', async ({server, client, player}) => {
                await server(async ({mp}) => {
                    player.seat.should.equal(1);
                });
            });

            it('should leave vehicle', async ({server, client, player}) => {
                await client(async ({mp}) => {
                    mp.game.task.leaveVehicle(player.handle, veh.handle, 0);
                    await tryFor(() => should().not.exist(player.vehicle), 4000);
                });
            ...
            it('should emit vehicle exit event', async ({server, client, player}) => {
                await client(async ({mp}) => {
                    await tryFor(() => exitVehicleEmittedTimes.should.equal(1));
                });
            ...
            it('should emit correct seat id in exit event', async ({server, client, player}) => {
                await client(async ({mp}) => {
                    exitVehicleSeat.should.equal(1);
                });
    ...
    describe('local', () => {
        describe('multiple vehicles', () => {
            const vehCount = 5;
            /** @type {(import('mp-client').mp.Vehicle)[]} */
            let vehs = [];
        ...
        describe('single vehicle', () => {
            ...
            it('should return correct remote id', async ({server, client}) => {
                await client(async ({mp}) => {
                    veh.remoteId.should.equal(65535);
                });
            });
            ...
            it('should return correct position', async ({server, client}) => {
                await client(async ({mp}) => {
                    veh.position = new mp.Vector3(4, 3, 73);
                    veh.position.x.should.be.approximately(4, 1);
                    veh.position.y.should.be.approximately(3, 1);
                    veh.position.z.should.be.approximately(73, 1);
                    veh.getCoords(false).x.should.be.approximately(4, 1);
                    veh.getCoords(false).y.should.be.approximately(3, 1);
                    veh.getCoords(false).z.should.be.approximately(73, 1);
                });
            });
            ...
            it('should return correct model', async ({server, client}) => {
                await client(async ({mp}) => {
                    veh.model.should.equal(mp.game.joaat('t20'));
                    veh.model = mp.game.joaat('neon');
                    veh.model.should.equal(mp.game.joaat('neon'));
                });
            });

            it('should restream on model change', async ({server, client}) => {
                await client(async ({mp}) => {
                    await tryFor(() => veh.handle.should.not.equal(0));
                    ...
                    veh.model = mp.game.joaat('neon');
                    veh.model.should.equal(mp.game.joaat('neon'));
                    await tryFor(() => veh.handle.should.not.equal(handle));
                });
            });
        ...
        describe(null, () => {
            ...
            it('should spawn invalid model properly', async ({server, client, player}) => {
                await client(async ({mp}) => {
                    const veh = mp.vehicles.new(mp.game.joaat('thismodeldoesnotexist'), new mp.Vector3(2, 2, 73), { dimension: player.dimension });
                    mp.vehicles.toArray().should.not.be.empty;
                    mp.vehicles.length.should.equal(1);
                    await tryFor(() => veh.handle.should.not.equal(0));
                });
            ...
            it('should create in dimension', async ({server, client, player}) => {
                await client(async ({mp}) => {
                    const veh = mp.vehicles.new(mp.game.joaat('t20'), new mp.Vector3(2, 2, 73), {
                        dimension: 6
                    });
                    veh.dimension.should.equal(6);
                });
            ...
            it('should create with alpha', async ({server, client, player}) => {
                await client(async ({mp}) => {
                    const veh = mp.vehicles.new(mp.game.joaat('t20'), new mp.Vector3(2, 2, 73), {
                    ...
                    await tryFor(() => veh.handle.should.not.equal(0));
                    veh.getAlpha().should.equal(64);
                });
            });

            it('should create with color', async ({server, client, player}) => {
                await client(async ({mp}) => {
                    const veh = mp.vehicles.new(mp.game.joaat('t20'), new mp.Vector3(2, 2, 73), {
                        dimension: player.dimension,
                        color: [5, 10]
            ...
            it('should create with custom color', async ({server, client, player}) => {
                await client(async ({mp}) => {
                    const veh = mp.vehicles.new(mp.game.joaat('t20'), new mp.Vector3(2, 2, 73), {
                        dimension: player.dimension,
                        color: [[255, 0, 128], [64, 64, 255]]
                    });
                    await tryFor(() => veh.handle.should.not.equal(0));
                    veh.getIsPrimaryColourCustom().should.be.true;
                    veh.getIsSecondaryColourCustom().should.be.true;
                    const color1 = veh.getCustomPrimaryColour(0, 0, 0);
                    const color2 = veh.getCustomSecondaryColour(0, 0, 0);
                    color1.r.should.equal(255);
                    color1.g.should.equal(0);
                    color1.b.should.equal(128);
                    color2.r.should.equal(64);
                    color2.g.should.equal(64);
                    color2.b.should.equal(255);
                });
            });

            it('should create with engine on', async ({server, client, player}) => {
                await client(async ({mp}) => {
                    const veh = mp.vehicles.new(mp.game.joaat('t20'), new mp.Vector3(2, 2, 73), {
                        dimension: player.dimension,
                        engine: true
            ...
            it('should create locked', async ({server, client, player}) => {
                await client(async ({mp}) => {
                    const veh = mp.vehicles.new(mp.game.joaat('t20'), new mp.Vector3(2, 2, 73), {
                        dimension: player.dimension,
                        locked: true
            ...
            it('should create with heading', async ({server, client, player}) => {
                await client(async ({mp}) => {
                    const veh = mp.vehicles.new(mp.game.joaat('t20'), new mp.Vector3(2, 2, 73), {
                        dimension: player.dimension,
                        heading: 90
                    ...
                    await tryFor(() => veh.handle.should.not.equal(0));
                    veh.getHeading().should.be.approximately(90, 3);
                });
            });

            it('should create with numberplate', async ({server, client, player}) => {
                await client(async ({mp}) => {
                    const veh = mp.vehicles.new(mp.game.joaat('t20'), new mp.Vector3(2, 2, 73), {
                        dimension: player.dimension,
                        numberPlate: 'zziger'
                    ...
                    await tryFor(() => veh.handle.should.not.equal(0));
                    veh.getNumberPlateText().trim().toLowerCase().should.equal('zziger');
                });
            });
        });
    });
