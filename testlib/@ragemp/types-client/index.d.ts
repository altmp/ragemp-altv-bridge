...
declare module 'mp-client' {
	...
	type MultiEventHandlers = Partial<IClientEvents> & Record<string, (...args: any) => void>;
	...
	interface PedMpBase extends EntityMp {
	...
	interface PedMp extends PedMpBase {
	...
	interface PlayerMp extends PedMpBase {
		armour: number;
		eyeColour: number;
		hairColour: number;
		...
		setVoiceFxChorus(fxHandle: VoiceHandle, {
			fWetDryMix,
			fDepth,
			fFeedback,
			fFrequency,
		...
		setVoiceFxCompressor(fxHandle: VoiceHandle, {
			fGain,
			fAttack,
			fRelease,
			fThreshold,
			fRatio,
			fPredelay
		}: VoiceFxCompressor): void;

		setVoiceFxDistortion(fxHandle: VoiceHandle, {
			fGain,
			fEdge,
			fPostEQCenterFrequency,
			fPostEQBandwidth,
			fPreLowpassCutoff
		}: VoiceFxDistortion): void;

		setVoiceFxEcho(fxHandle: VoiceHandle, {
			fWetDryMix,
			fFeedback,
			fLeftDelay,
			fRightDelay,
		...
		setVoiceFxFlanger(fxHandle: VoiceHandle, {
			fWetDryMix,
			fDepth,
			fFeedback,
			fFrequency,
		...
		setVoiceFxI3DL2Reverb(fxHandle: VoiceHandle, {
			lRoom,
			lRoomHF,
			flRoomRolloffFactor,
			flDecayTime,
