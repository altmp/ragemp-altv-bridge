...
import type { Node, Identifier, FunctionExpression } from "estree";
...
function registerNativeFunction(id: Identifier, fn: FunctionExpression, invoker: NativeInvoker) {
    ...
    for (const el of fn.body.body) {
        ...
        if (el.type === "ExpressionStatement"
            ...
            const argId = Math.floor((+el.expression.right.property.raw - 2) / invoker.alignment);
            ...
            if (key.endsWith(".x")) {
                ...
                parsed.resObject[key] = `$res[${refId}]`;
            }
        }
        ...
        if (el.type === "ReturnStatement") {
            const expr = walk(el.argument, {
                enter(node) {
                    ...
                    if (node.type === "Identifier" && node.name === "resultString") {
...
esprima.parseScript(source, {}, (node) => {
    ...
    if (node.type === "VariableDeclarator" && node.id.type === "Identifier") {
        ...
        if (node.init?.type === "MemberExpression"
            ...
            outputCode += `if (!${prototype}) ${prototype} = {};\n`;
            outputCode += `if (!${prototype}.prototype) ${prototype}.prototype = {};\n`;
        }
    ...
    if (node.type === "ExpressionStatement"
        ...
        for (let i = 2; i < chunks.length; i++) {
            ...
            outputCode += `if (!${currName}) ${currName} = {};\n`;
        }
